...
int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    char pattern[256];
    char value[512];
    const size_t pattern_actual = pattern_length < sizeof(pattern) - 1 ? pattern_length : sizeof(pattern) - 1;
    ...
    const size_t value_actual = value_available < sizeof(value) - 1 ? value_available : sizeof(value) - 1;
...
int main(int argc, char** argv)
{
    ...
    if (!bc_core_parse_unsigned_integer_64_decimal(argv[1], iterations_length, &iterations, &consumed) || consumed != iterations_length) {
    ...
    if (argc >= 3) {
        ...
        if (!bc_core_parse_unsigned_integer_64_decimal(argv[2], seed_length, &seed, &consumed) || consumed != seed_length) {
