...
static bool bc_seek_application_run(const bc_runtime_t* application, void* user_data)
{
    ...
    if (effective_workers >= 2 && state->cli_options.threads_mode != BC_SEEK_THREADS_MODE_MONO) {
        ...
        walk_ok =
            bc_seek_discovery_walk_sequential(memory_context, &state->predicate, root_paths, root_count, state->cli_options.follow_symlinks,
                                              state->cli_options.one_file_system, &state->output, state->errors, signal_handler);
    }
...
int main(int argument_count, char** argument_values)
{
    ...
    bc_allocators_context_config_t cli_memory_config = {
        .max_pool_memory = 0, .tracking_enabled = true, .leak_callback = NULL, .leak_callback_argument = NULL};
