...
static void bc_seek_emit_stderr_message(const char *prefix, const char *middle,
...
static bool bc_seek_application_init(const bc_runtime_t *application,
                                     void *user_data) {
  ...
  if (state->cli_options.output_mode == BC_SEEK_OUTPUT_MODE_STDOUT) {
    if (!bc_seek_output_open_stdout(state->cli_options.null_terminated,
                                    &state->output)) {
    ...
    if (!bc_seek_output_open_file(state->cli_options.output_path,
                                  state->cli_options.null_terminated,
                                  &state->output)) {
      bc_seek_emit_stderr_message("bc-seek: cannot open output '", NULL,
...
static bool bc_seek_application_run(const bc_runtime_t *application,
                                    void *user_data) {
  bc_seek_application_state_t *state = (bc_seek_application_state_t *)user_data;
  ...
  if (effective_workers >= 2 &&
      state->cli_options.threads_mode != BC_SEEK_THREADS_MODE_MONO) {
    walk_ok = bc_seek_discovery_walk_parallel(
        memory_context, concurrency_context, &state->predicate, root_paths,
        root_count, state->cli_options.follow_symlinks,
        state->cli_options.one_file_system, &state->output, state->errors,
        signal_handler);
  } else {
    walk_ok = bc_seek_discovery_walk_sequential(
        memory_context, &state->predicate, root_paths, root_count,
        state->cli_options.follow_symlinks, state->cli_options.one_file_system,
        &state->output, state->errors, signal_handler);
  }
  ...
  state->exit_code =
      bc_runtime_error_collector_count(state->errors) == 0 ? 0 : 1;
  return true;
}

static void bc_seek_application_cleanup(const bc_runtime_t *application,
                                        void *user_data) {
  bc_seek_application_state_t *state = (bc_seek_application_state_t *)user_data;
...
int main(int argument_count, char **argument_values) {
  ...
  if (!bc_runtime_config_store_create(cli_memory_context, &cli_store)) {
    bc_seek_emit_stderr_message(
        "bc-seek: failed to initialize CLI config store\n", NULL, NULL, NULL);
  ...
  if (state.cli_options.threads_mode == BC_SEEK_THREADS_MODE_MONO) {
    ...
    parallel_config.worker_count =
        state.cli_options.explicit_worker_count > 0
            ? state.cli_options.explicit_worker_count - 1
            : 0;
    ...
    if (state.cli_options.explicit_worker_count > logical_processor_count) {
      bc_seek_emit_stderr_message(
          "bc-seek: --threads exceeds online logical processors\n", NULL, NULL,
          NULL);
      bc_runtime_config_store_destroy(cli_memory_context, cli_store);
      bc_allocators_context_destroy(cli_memory_context);
      return 2;
    }
    if (state.cli_options.explicit_worker_count >
        bc_concurrency_physical_core_count()) {
