...
static bool bc_seek_cli_parse_unsigned(const char *value, size_t length,
                                       uint64_t *out_value) {
  if (length == 0) {
    return false;
  }
...
bool bc_seek_cli_parse_type_filter(const char *value,
                                   bc_seek_entry_type_t *out_type) {
  if (bc_seek_strings_equal(value, "f") ||
      bc_seek_strings_equal(value, "file")) {
    *out_type = BC_SEEK_ENTRY_TYPE_FILE;
    return true;
  }
  if (bc_seek_strings_equal(value, "d") ||
      bc_seek_strings_equal(value, "dir")) {
    *out_type = BC_SEEK_ENTRY_TYPE_DIRECTORY;
    return true;
  }
  if (bc_seek_strings_equal(value, "l") ||
      bc_seek_strings_equal(value, "link") ||
      bc_seek_strings_equal(value, "symlink")) {
...
bool bc_seek_cli_parse_size_filter(const char *value,
                                   bc_seek_compare_op_t *out_op,
                                   uint64_t *out_threshold_bytes) {
...
bool bc_seek_cli_parse_mtime_filter(const char *value,
                                    bc_seek_compare_op_t *out_op,
                                    int64_t *out_days) {
  if (value == NULL || value[0] == '\0') {
    return false;
...
bool bc_seek_cli_parse_threads(const char *value,
  ...
  size_t consumed = 0;
  if (!bc_core_parse_unsigned_integer_64_decimal(value, value_length,
                                                 &parsed_value, &consumed)) {
    return false;
  }
