...
static void bc_seek_cli_emit_stderr(const char *prefix, const char *middle,
...
static const char *const bc_seek_type_allowed_values[] = {
    "f", "d", "l", "file", "dir", "link", "symlink", NULL};
...
static const bc_runtime_cli_option_spec_t bc_seek_global_options[] = {
    {
        ...
        .help_summary =
            "thread mode: mono (single-thread, alias 0), auto (CPU-bound, "
            "physical cores - 1, default), io (I/O-bound, logical processors - "
            "1, oversubscribe), or N (1..logical_cpu_count)",
    },
};
...
static const bc_runtime_cli_option_spec_t bc_seek_find_options[] = {
    ...
    },
    {
        ...
        .help_summary =
            "filter by size (find(1) syntax; default unit: 512-byte blocks)",
    },
    {
        .long_name = "mtime",
        .type = BC_RUNTIME_CLI_OPTION_STRING,
        ...
        .help_summary =
            "filter by modification age in 24h units (find(1) semantics)",
    },
    {
        .long_name = "newer",
        .type = BC_RUNTIME_CLI_OPTION_STRING,
        ...
        .help_summary =
            "match entries modified more recently than the reference file",
    },
    {
        .long_name = "perm",
        .type = BC_RUNTIME_CLI_OPTION_STRING,
        ...
        .help_summary =
            "match entries whose permission bits equal the octal mask",
    },
    {
        .long_name = "max-depth",
    ...
    },
    {
        ...
        .value_placeholder = NULL,
        .help_summary =
            "do not prune well-known directories (.git, node_modules, ...)",
    },
    {
...
static const bc_runtime_cli_command_spec_t bc_seek_commands[] = {
    {
        ...
        .option_count =
            sizeof(bc_seek_find_options) / sizeof(bc_seek_find_options[0]),
...
static const bc_runtime_cli_program_spec_t bc_seek_program_spec_value = {
    ...
    .global_option_count =
        sizeof(bc_seek_global_options) / sizeof(bc_seek_global_options[0]),
...
bool bc_seek_cli_bind_global_threads(const bc_runtime_config_store_t *store,
                                     bc_seek_threads_mode_t *out_mode,
  ...
  if (!bc_seek_cli_parse_threads(threads_value, out_mode, out_worker_count)) {
    bc_seek_cli_emit_stderr("bc-seek: invalid --threads value: '", NULL,
                            threads_value, "'\n");
    return false;
  }
  return true;
...
static bool bc_seek_cli_lookup_string(const bc_runtime_config_store_t *store,
...
static bool bc_seek_cli_lookup_integer(const bc_runtime_config_store_t *store,
                                       const char *key, long *out_value) {
  return bc_runtime_config_store_get_integer(store, key, out_value);
}

static bool bc_seek_cli_lookup_boolean(const bc_runtime_config_store_t *store,
                                       const char *key, bool *out_value) {
  return bc_runtime_config_store_get_boolean(store, key, out_value);
}

static bool bc_seek_cli_bind_output(const char *value,
                                    bc_seek_output_mode_t *out_mode,
                                    const char **out_path) {
...
static bool bc_seek_cli_bind_newer_reference(const char *path,
                                             time_t *out_mtime);

bool bc_seek_cli_bind_options(const bc_runtime_config_store_t *store,
  ...
  if (!bc_seek_cli_bind_global_threads(store, &out_options->threads_mode,
  ...
  if (has_name && has_iname) {
    bc_seek_cli_emit_stderr(
        "bc-seek: --name and --iname are mutually exclusive\n", NULL, NULL,
        NULL);
    return false;
  }
  if (has_name) {
  ...
  if (bc_seek_cli_lookup_string(store, "find.size", &size_value)) {
    if (!bc_seek_cli_parse_size_filter(size_value, &out_options->size_op,
                                       &out_options->size_threshold_bytes)) {
  ...
  if (bc_seek_cli_lookup_string(store, "find.mtime", &mtime_value)) {
    int64_t days = 0;
    if (!bc_seek_cli_parse_mtime_filter(mtime_value, &out_options->mtime_op,
                                        &days)) {
    ...
    if (days > (int64_t)(INT64_MAX / 86400)) {
      bc_seek_cli_emit_stderr("bc-seek: --mtime value out of range: '", NULL,
                              mtime_value, "'\n");
      return false;
    }
  ...
  if (bc_seek_cli_lookup_string(store, "find.newer", &newer_value)) {
    ...
    if (!bc_seek_cli_bind_newer_reference(newer_value, &reference_mtime)) {
      bc_seek_cli_emit_stderr("bc-seek: cannot stat --newer reference: '", NULL,
                              newer_value, "'\n");
      return false;
    }
  ...
  if (out_options->has_max_depth &&
      out_options->min_depth > out_options->max_depth) {
  ...
  if (!bc_runtime_config_store_get_string(store, "find.output",
                                          &output_value)) {
    bc_seek_cli_emit_stderr("bc-seek: internal error: missing find.output\n",
                            NULL, NULL, NULL);
    return false;
  }
  if (!bc_seek_cli_bind_output(output_value, &out_options->output_mode,
                               &out_options->output_path)) {
