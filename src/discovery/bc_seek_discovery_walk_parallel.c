...
{
    bc_seek_parallel_worker_slot_t* slot = (bc_seek_parallel_worker_slot_t*)bc_concurrency_worker_slot(context->worker_slot_index);
    if (slot == NULL) {
        return false;
    }
    if (!slot->initialized) {
...
static bool bc_seek_parallel_visit(const bc_io_walk_entry_t* entry, void* user_data)
{
    ...
    if (context->one_file_system) {
        ...
        if (atomic_load_explicit(&context->root_device_initialized, memory_order_acquire) == 1u && entry->device_id != expected_root) {
            return true;
        }
    }
...
static bool bc_seek_parallel_should_descend(const bc_io_walk_entry_t* entry, void* user_data)
{
    ...
    if (context->one_file_system && atomic_load_explicit(&context->root_device_initialized, memory_order_acquire) == 1u &&
...
static bool bc_seek_parallel_handle_root(bc_seek_parallel_context_t* context, bc_allocators_context_t* memory_context,
                                         bc_concurrency_context_t* concurrency_context, bc_runtime_signal_handler_t* signal_handler,
                                         bc_seek_parallel_root_result_t* root_result, const char* root_path)
{
...
bool bc_seek_discovery_walk_parallel(bc_allocators_context_t* memory_context, bc_concurrency_context_t* concurrency_context,
                                     ...
                                     bc_runtime_error_collector_t* errors, const bc_runtime_signal_handler_t* signal_handler)
{
    if (concurrency_context == NULL || bc_concurrency_effective_worker_count(concurrency_context) < 2u) {
        return bc_seek_discovery_walk_sequential(memory_context, predicate, root_paths, root_count, follow_symlinks, one_file_system,
                                                 output, errors, signal_handler);
    }
    ...
    if (root_count == 0) {
        ...
        if (!bc_seek_parallel_handle_root(&context, memory_context, concurrency_context, (bc_runtime_signal_handler_t*)signal_handler,
