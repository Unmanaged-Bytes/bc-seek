...
typedef struct bc_seek_parallel_worker_slot {
  ...
  bool initialized;
  BC_PAD_TO_CACHE_LINE(sizeof(bc_seek_output_bytes_t) + sizeof(size_t) +
                       sizeof(bool));
...
static bool
bc_seek_parallel_append_match(bc_allocators_context_t *memory_context,
                              bc_seek_output_bytes_t *buffer, const char *path,
                              size_t path_length, char separator) {
  if (!bc_seek_output_bytes_append_bulk(memory_context, buffer, path,
                                        path_length)) {
    return false;
  }
...
static bool
bc_seek_parallel_ensure_worker_slot(const bc_seek_parallel_context_t *context,
                                    bc_allocators_context_t *worker_memory,
                                    bc_seek_parallel_worker_slot_t **out_slot) {
  bc_seek_parallel_worker_slot_t *slot =
      (bc_seek_parallel_worker_slot_t *)bc_concurrency_worker_slot(
  ...
  if (!slot->initialized) {
    ...
    if (!bc_seek_output_bytes_reserve(
            worker_memory, &slot->output_bytes,
            BC_SEEK_PARALLEL_OUTPUT_INITIAL_CAPACITY)) {
      return false;
    }
...
static void
bc_seek_parallel_record_error(bc_runtime_error_collector_t *errors,
                              bc_allocators_context_t *memory_context,
                              const char *path, int errno_value) {
  (void)bc_runtime_error_collector_append(errors, memory_context, path, NULL,
                                          errno_value);
}
...
static bc_seek_entry_type_t
bc_seek_parallel_type_from_walk_kind(bc_io_walk_entry_kind_t kind) {
  switch (kind) {
...
static const char *bc_seek_parallel_basename(const char *path,
                                             size_t path_length,
                                             size_t *out_length) {
 ...
 * bc_io_walk_visit_fn */
static bool bc_seek_parallel_visit(const bc_io_walk_entry_t *entry,
                                   void *user_data) {
  bc_seek_parallel_context_t *context = (bc_seek_parallel_context_t *)user_data;

  if (context->one_file_system) {
    dev_t expected_root =
        atomic_load_explicit(&context->root_device, memory_order_acquire);
    if (atomic_load_explicit(&context->root_device_initialized,
  ...
  const char *basename = bc_seek_parallel_basename(
      entry->absolute_path, entry->absolute_path_length, &basename_length);
  ...
  bc_seek_filter_decision_t decision =
      bc_seek_filter_evaluate(context->predicate, &candidate);
  if (decision != BC_SEEK_FILTER_DECISION_ACCEPT) {
    return true;
  }

  if (!bc_seek_parallel_append_match(
          worker_memory, &slot->output_bytes, entry->absolute_path,
          entry->absolute_path_length, context->output_separator)) {
    bc_seek_parallel_record_error(context->errors, context->main_memory_context,
                                  entry->absolute_path, ENOMEM);
    return true;
  }
...
static bool bc_seek_parallel_should_descend(const bc_io_walk_entry_t *entry,
                                            void *user_data) {
  const bc_seek_parallel_context_t *context =
      (const bc_seek_parallel_context_t *)user_data;

  if (context->predicate->has_max_depth &&
      entry->depth >= context->predicate->max_depth) {
    return false;
  }
  ...
  if (context->predicate->respect_ignore_defaults &&
      bc_seek_filter_ignored_directory_name(basename, basename_length)) {
    return false;
  }
  if (!context->predicate->include_hidden &&
      bc_seek_parallel_is_hidden_name(basename)) {
    return false;
  }

  if (context->one_file_system &&
      atomic_load_explicit(&context->root_device_initialized,
                           memory_order_acquire) == 1u &&
      entry->device_id !=
          atomic_load_explicit(&context->root_device, memory_order_acquire)) {
    return false;
...
/* cppcheck-suppress constParameterCallback; signature fixed by
 * bc_io_walk_error_fn */
static void bc_seek_parallel_on_error(const char *path, const char *stage,
...
static void bc_seek_parallel_collect_slot_iovec(void *slot_data,
                                                size_t worker_index,
                                                void *arg) {
  (void)worker_index;
  const bc_seek_parallel_worker_slot_t *slot =
      (const bc_seek_parallel_worker_slot_t *)slot_data;
  bc_seek_parallel_iovec_builder_t *builder =
      (bc_seek_parallel_iovec_builder_t *)arg;
  if (!slot->initialized || slot->output_bytes.length == 0u ||
      builder->count >= BC_SEEK_PARALLEL_MAX_IOVEC) {
    return;
  }
...
static bool bc_seek_parallel_write_all_iovec(int fd, struct iovec *iov,
                                             size_t count) {
  while (count > 0u) {
...
/* cppcheck-suppress constParameterCallback; signature fixed by
 * bc_io_walk_should_stop_fn */
static bool bc_seek_parallel_should_stop_check(void *user_data) {
  const bc_runtime_signal_handler_t *handler =
      (const bc_runtime_signal_handler_t *)user_data;
  if (handler == NULL) {
    return false;
  }
...
static bool
bc_seek_parallel_handle_root(bc_seek_parallel_context_t *context,
                             bc_allocators_context_t *memory_context,
  ...
  if (fstatat(AT_FDCWD, root_path, &root_stat, stat_flags) != 0) {
    bc_seek_parallel_record_error(context->errors, memory_context, root_path,
                                  errno);
    return false;
  }
  ...
  if (context->one_file_system) {
    ...
    if (atomic_compare_exchange_strong_explicit(
            &context->root_device_initialized, &expected, 1u,
            memory_order_acq_rel, memory_order_acquire)) {
      atomic_store_explicit(&context->root_device, root_stat.st_dev,
  ...
  size_t basename_length = 0;
  const char *basename =
      bc_seek_parallel_basename(root_path, root_path_length, &basename_length);
  ...
  root_candidate.size_bytes =
      root_stat.st_size >= 0 ? (uint64_t)root_stat.st_size : 0u;
  ...
  bc_seek_filter_decision_t decision =
      bc_seek_filter_evaluate(context->predicate, &root_candidate);
  if (decision == BC_SEEK_FILTER_DECISION_ACCEPT) {
    if (!bc_seek_parallel_append_match(
            memory_context, &root_result->main_output_buffer, root_path,
            root_path_length, context->output_separator)) {
      bc_seek_parallel_record_error(context->errors, memory_context, root_path,
                                    ENOMEM);
    } else {
  ...
  if (context->predicate->has_max_depth &&
      context->predicate->max_depth == 0u) {
    return true;
  }
  ...
  bc_io_walk_config_t walk_config = {
      ...
      .should_stop_check = bc_seek_parallel_should_stop_check,
...
bool bc_seek_discovery_walk_parallel(
    bc_allocators_context_t *memory_context,
    bc_concurrency_context_t *concurrency_context,
    const bc_seek_predicate_t *predicate, const char *const *root_paths,
    size_t root_count, bool follow_symlinks, bool one_file_system,
    bc_seek_output_t *output, bc_runtime_error_collector_t *errors,
    const bc_runtime_signal_handler_t *signal_handler) {
  if (concurrency_context == NULL ||
      bc_concurrency_effective_worker_count(concurrency_context) < 2u) {
    return bc_seek_discovery_walk_sequential(
        memory_context, predicate, root_paths, root_count, follow_symlinks,
        one_file_system, output, errors, signal_handler);
  }
  ...
  atomic_store_explicit(&context.root_device_initialized, 0u,
  ...
  if (!bc_seek_output_bytes_reserve(memory_context,
                                    &root_result.main_output_buffer,
                                    BC_SEEK_PARALLEL_OUTPUT_INITIAL_CAPACITY)) {
    return false;
  }
  ...
  if (root_count == 0) {
    ...
    if (!bc_seek_parallel_handle_root(&context, memory_context,
                                      concurrency_context,
                                      (bc_runtime_signal_handler_t *)signal_handler,
    ...
    for (size_t root_index = 0; root_index < root_count; root_index++) {
      if (!bc_seek_parallel_handle_root(
              &context, memory_context, concurrency_context,
              (bc_runtime_signal_handler_t *)signal_handler, &root_result,
              root_paths[root_index])) {
        any_root_ok = false;
      }
    }
  }
  ...
  if (root_result.main_output_buffer.length > 0u &&
      builder.count < BC_SEEK_PARALLEL_MAX_IOVEC) {
  ...
  bc_concurrency_foreach_slot(concurrency_context, context.worker_slot_index,
                              bc_seek_parallel_collect_slot_iovec, &builder);
  ...
  if (builder.count > 0u) {
    ...
    merge_ok = merge_ok && bc_seek_parallel_write_all_iovec(
                               output->fd, builder.iov, builder.count);
  }
