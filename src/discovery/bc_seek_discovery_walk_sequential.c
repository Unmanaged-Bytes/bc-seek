...
static bool bc_seek_walk_should_stop(bc_seek_walk_context_t* context)
{
    ...
    if (context->signal_handler != NULL) {
        ...
        if (bc_runtime_signal_handler_should_stop(context->signal_handler, &should_stop) && should_stop) {
...
static bool bc_seek_walk_should_descend_directory(const bc_seek_walk_context_t* context, const char* name, size_t name_length,
                                                  size_t child_depth)
{
...
static bool bc_seek_walk_iterate_entries(bc_seek_walk_context_t* context, int directory_fd, char* path_buffer, size_t path_length,
                                         size_t depth)
{
