...
static bool bc_seek_walk_should_stop(bc_seek_walk_context_t *context) {
  ...
  if (context->signal_handler != NULL) {
    ...
    if (bc_runtime_signal_handler_should_stop(context->signal_handler,
                                              &should_stop) &&
        should_stop) {
...
static bool bc_seek_walk_append_path(char *buffer, size_t buffer_capacity,
                                     size_t *path_length, const char *name,
                                     size_t name_length) {
  size_t current_length = *path_length;
  bool needs_separator =
      current_length > 0 && buffer[current_length - 1] != '/';
...
static void bc_seek_walk_truncate(char *buffer, size_t *path_length,
...
static bc_seek_entry_type_t bc_seek_walk_type_from_dtype(unsigned char d_type,
                                                         bool *out_resolved) {
  switch (d_type) {
  case DT_REG:
    *out_resolved = true;
...
static bool bc_seek_walk_process_directory(bc_seek_walk_context_t *context,
                                           int parent_directory_fd,
                                           char *path_buffer,
                                           size_t path_length, size_t depth);

static bool bc_seek_walk_emit_if_match(bc_seek_walk_context_t *context,
  ...
  return bc_seek_output_emit(context->output, candidate->path,
                             candidate->path_length);
}

static bool
bc_seek_walk_should_descend_directory(const bc_seek_walk_context_t *context,
                                      const char *name, size_t name_length,
                                      size_t child_depth) {
  if (context->predicate->has_max_depth &&
      child_depth > context->predicate->max_depth) {
    return false;
  }
  if (context->predicate->respect_ignore_defaults &&
      bc_seek_filter_ignored_directory_name(name, name_length)) {
    return false;
  }
  if (!context->predicate->include_hidden &&
      bc_seek_walk_is_hidden_name(name)) {
    return false;
  }
  return true;
}

static bool bc_seek_walk_open_subdir(const bc_seek_walk_context_t *context,
                                     int parent_directory_fd, const char *name,
                                     int *out_fd) {
...
static bool bc_seek_walk_iterate_entries(bc_seek_walk_context_t *context,
                                         int directory_fd, char *path_buffer,
                                         size_t path_length, size_t depth) {
  bc_io_dirent_reader_t *reader = NULL;
  if (!bc_io_dirent_reader_create(context->memory_context, directory_fd,
                                  &reader)) {
    bc_runtime_error_collector_append(context->errors, context->memory_context,
                                      path_buffer, NULL, ENOMEM);
    return false;
  }
  ...
  for (;;) {
    ...
    if (!bc_io_dirent_reader_next(reader, &entry, &has_entry)) {
      ...
      bc_runtime_error_collector_append(context->errors,
                                        context->memory_context, path_buffer,
                                        NULL, reader_errno);
      ok = false;
      break;
    }
    ...
    if (!context->predicate->include_hidden &&
        bc_seek_walk_is_hidden_name(entry.name)) {
      continue;
    }

    size_t saved_path_length = path_length;
    if (!bc_seek_walk_append_path(path_buffer, BC_SEEK_DISCOVERY_PATH_CAPACITY,
                                  &path_length, entry.name,
                                  entry.name_length)) {
    ...
    bc_seek_entry_type_t entry_type =
        bc_seek_walk_type_from_dtype(entry.d_type, &type_resolved);
    const char *basename_pointer =
        path_buffer + path_length - entry.name_length;
    ...
    bool is_directory = candidate.type_resolved &&
    ...
    if (is_directory &&
        bc_seek_walk_should_descend_directory(
            context, entry.name, entry.name_length, candidate.depth)) {
      int subdir_fd = -1;
      if (!bc_seek_walk_open_subdir(context, directory_fd, entry.name,
                                    &subdir_fd)) {
        bc_runtime_error_collector_append(
            context->errors, context->memory_context, path_buffer, NULL, errno);
      } else {
        bool recurse_ok = bc_seek_walk_process_directory(
            context, subdir_fd, path_buffer, path_length, candidate.depth);
        close(subdir_fd);
...
static bool bc_seek_walk_process_directory(bc_seek_walk_context_t *context,
                                           ...
                                           char *path_buffer,
                                           size_t path_length, size_t depth) {
  return bc_seek_walk_iterate_entries(context, parent_directory_fd, path_buffer,
                                      path_length, depth);
}

static bool bc_seek_walk_visit_root(bc_seek_walk_context_t *context,
  ...
  if (!bc_core_length(root_path, '\0', &path_length)) {
    bc_runtime_error_collector_append(context->errors, context->memory_context,
                                      root_path, NULL, EINVAL);
    return false;
  }
  if (path_length >= sizeof(path_buffer)) {
    bc_runtime_error_collector_append(context->errors, context->memory_context,
                                      root_path, NULL, ENAMETOOLONG);
    return false;
  }
  ...
  size_t basename_length =
      path_length - (size_t)(basename_pointer - path_buffer);
  ...
  bool walk_ok = bc_seek_walk_process_directory(context, root_fd, path_buffer,
                                                path_length, 0);
  close(root_fd);
  return walk_ok;
}

bool bc_seek_discovery_walk_sequential(
    bc_allocators_context_t *memory_context,
