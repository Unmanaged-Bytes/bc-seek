...
static const char* const bc_seek_ignored_directory_names[] = {
    ".git",        ".hg",   ".svn", ".bzr",        "CVS",           ".cache",      "node_modules", "target",
    "__pycache__", ".venv", ".tox", ".mypy_cache", ".pytest_cache", ".ruff_cache", ".ccls-cache",  NULL,
};
...
static bc_seek_filter_decision_t bc_seek_filter_check_path(const bc_seek_predicate_t* predicate, const bc_seek_candidate_t* candidate)
{
    ...
    return bc_seek_filter_glob_matches(predicate->path_glob, candidate->path, false) ? BC_SEEK_FILTER_DECISION_ACCEPT
                                                                                     : BC_SEEK_FILTER_DECISION_REJECT;
}
