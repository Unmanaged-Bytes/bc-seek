...
static const char *const bc_seek_ignored_directory_names[] = {
    ".git",          ".hg",         ".svn",         ".bzr",
    "CVS",           ".cache",      "node_modules", "target",
    "__pycache__",   ".venv",       ".tox",         ".mypy_cache",
    ".pytest_cache", ".ruff_cache", ".ccls-cache",  NULL,
};

bool bc_seek_filter_ignored_directory_name(const char *name,
                                           size_t name_length) {
  for (size_t index = 0; bc_seek_ignored_directory_names[index] != NULL;
       index++) {
...
bool bc_seek_filter_glob_matches(const char *pattern, const char *value,
...
static bool bc_seek_filter_compare(uint64_t left, uint64_t right,
                                   bc_seek_compare_op_t op) {
  switch (op) {
...
static bool bc_seek_filter_compare_signed(int64_t left, int64_t right,
                                          bc_seek_compare_op_t op) {
  switch (op) {
  case BC_SEEK_COMPARE_GREATER:
    return left > right;
...
bool bc_seek_predicate_from_options(const bc_seek_cli_options_t *options,
                                    bc_seek_predicate_t *out_predicate) {
  ...
  out_predicate->mtime_threshold_seconds_ago =
      options->mtime_threshold_seconds_ago;
  ...
  out_predicate->require_stat =
      options->has_size_filter || options->has_mtime_filter ||
      options->has_newer_reference || options->has_perm_filter;
...
bool bc_seek_filter_populate_stat(bc_seek_candidate_t *candidate) {
  ...
  int rc = fstatat(candidate->parent_directory_fd, candidate->basename,
                   &entry_stat, flags);
  if (rc != 0) {
    return false;
  }
  ...
  candidate->size_bytes =
      entry_stat.st_size >= 0 ? (uint64_t)entry_stat.st_size : 0u;
...
static bc_seek_filter_decision_t
bc_seek_filter_check_depth(const bc_seek_predicate_t *predicate,
                           const bc_seek_candidate_t *candidate) {
...
static bc_seek_filter_decision_t
bc_seek_filter_check_type(const bc_seek_predicate_t *predicate,
                          const bc_seek_candidate_t *candidate) {
  ...
  return candidate->entry_type == predicate->type_filter
             ? BC_SEEK_FILTER_DECISION_ACCEPT
             : BC_SEEK_FILTER_DECISION_REJECT;
}

static bc_seek_filter_decision_t
bc_seek_filter_check_name(const bc_seek_predicate_t *predicate,
                          const bc_seek_candidate_t *candidate) {
  if (!predicate->has_name_glob) {
    return BC_SEEK_FILTER_DECISION_ACCEPT;
  }
  return bc_seek_filter_glob_matches(predicate->name_glob, candidate->basename,
                                     predicate->name_case_insensitive)
             ? BC_SEEK_FILTER_DECISION_ACCEPT
             : BC_SEEK_FILTER_DECISION_REJECT;
}

static bc_seek_filter_decision_t
bc_seek_filter_check_path(const bc_seek_predicate_t *predicate,
                          const bc_seek_candidate_t *candidate) {
  if (!predicate->has_path_glob) {
    return BC_SEEK_FILTER_DECISION_ACCEPT;
  }
  return bc_seek_filter_glob_matches(predicate->path_glob, candidate->path,
                                     false)
             ? BC_SEEK_FILTER_DECISION_ACCEPT
             : BC_SEEK_FILTER_DECISION_REJECT;
}
...
static bc_seek_filter_decision_t
bc_seek_filter_check_size(const bc_seek_predicate_t *predicate,
                          const bc_seek_candidate_t *candidate) {
  ...
  return bc_seek_filter_compare(candidate->size_bytes,
                                predicate->size_threshold_bytes,
                                predicate->size_op)
             ? BC_SEEK_FILTER_DECISION_ACCEPT
             : BC_SEEK_FILTER_DECISION_REJECT;
}
...
static bc_seek_filter_decision_t
bc_seek_filter_check_mtime(const bc_seek_predicate_t *predicate,
                           const bc_seek_candidate_t *candidate) {
  ...
  int64_t seconds_ago = (int64_t)predicate->evaluation_reference_time -
                        (int64_t)candidate->modification_time;
  int64_t threshold = predicate->mtime_threshold_seconds_ago;
  return bc_seek_filter_compare_signed(seconds_ago, threshold,
                                       predicate->mtime_op)
             ? BC_SEEK_FILTER_DECISION_ACCEPT
             : BC_SEEK_FILTER_DECISION_REJECT;
}
...
static bc_seek_filter_decision_t
bc_seek_filter_check_newer(const bc_seek_predicate_t *predicate,
                           const bc_seek_candidate_t *candidate) {
  ...
  return candidate->modification_time > predicate->newer_reference_mtime
             ? BC_SEEK_FILTER_DECISION_ACCEPT
             : BC_SEEK_FILTER_DECISION_REJECT;
}

static bc_seek_filter_decision_t
bc_seek_filter_check_perm(const bc_seek_predicate_t *predicate,
                          const bc_seek_candidate_t *candidate) {
  ...
  return (candidate->permission_mask & 07777u) ==
                 (predicate->perm_mask & 07777u)
             ? BC_SEEK_FILTER_DECISION_ACCEPT
             : BC_SEEK_FILTER_DECISION_REJECT;
}

bc_seek_filter_decision_t
bc_seek_filter_evaluate(const bc_seek_predicate_t *predicate,
                        bc_seek_candidate_t *candidate) {
  bc_seek_filter_decision_t decision =
      bc_seek_filter_check_depth(predicate, candidate);
