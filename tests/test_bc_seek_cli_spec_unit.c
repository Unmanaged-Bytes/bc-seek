...
static bc_runtime_cli_parse_status_t parse_into(fixture_t* fixture, const char* const argv[], int argc, bc_runtime_cli_parsed_t* out_parsed)
{
    ...
    bc_runtime_cli_parse_status_t status =
        bc_runtime_cli_parse(bc_seek_cli_program_spec(), argc, argv, fixture->store, out_parsed, error_stream);
...
static void test_bind_options_all_predicates(void** state)
{
    fixture_t* fixture = *state;
    const char* argv[] = {
        "bc-seek",       "find",     "--name=*.c",  "--type=f",          "--size=+1k",        "--mtime=-7", "--perm=755", "--max-depth=3",
        "--min-depth=1", "--hidden", "--no-ignore", "--follow-symlinks", "--one-file-system", "--null",     "--output=-", "/tmp",
    };
    bc_runtime_cli_parsed_t parsed;
