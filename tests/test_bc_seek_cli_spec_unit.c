...
static int setup(void **state) {
  ...
  if (!bc_runtime_config_store_create(fixture->memory_context,
                                      &fixture->store)) {
...
static bc_runtime_cli_parse_status_t
parse_into(fixture_t *fixture, const char *const argv[], int argc,
           bc_runtime_cli_parsed_t *out_parsed) {
  FILE *error_stream = fmemopen(NULL, 4096, "w");
  bc_runtime_cli_parse_status_t status =
      bc_runtime_cli_parse(bc_seek_cli_program_spec(), argc, argv,
                           fixture->store, out_parsed, error_stream);
...
static void test_bind_global_threads_auto(void **state) {
  ...
  assert_int_equal(parse_into(fixture, argv, 3, &parsed),
...
static void test_bind_global_threads_mono(void **state) {
  ...
  bc_runtime_cli_parsed_t parsed;
  assert_int_equal(parse_into(fixture, argv, 4, &parsed),
                   BC_RUNTIME_CLI_PARSE_OK);
...
static void test_bind_options_defaults(void **state) {
  ...
  const char *argv[] = {"bc-seek", "find"};
  bc_runtime_cli_parsed_t parsed;
  assert_int_equal(parse_into(fixture, argv, 2, &parsed),
                   BC_RUNTIME_CLI_PARSE_OK);
...
static void test_bind_options_all_predicates(void **state) {
  fixture_t *fixture = *state;
  const char *argv[] = {
      "bc-seek",           "find",       "--name=*.c",  "--type=f",
      "--size=+1k",        "--mtime=-7", "--perm=755",  "--max-depth=3",
      "--min-depth=1",     "--hidden",   "--no-ignore", "--follow-symlinks",
      "--one-file-system", "--null",     "--output=-",  "/tmp",
  };
  bc_runtime_cli_parsed_t parsed;
  assert_int_equal(
      parse_into(fixture, argv, sizeof(argv) / sizeof(argv[0]), &parsed),
      BC_RUNTIME_CLI_PARSE_OK);
  bc_seek_cli_options_t options;
  assert_true(bc_seek_cli_bind_options(fixture->store, &parsed, &options));
...
static void test_bind_options_name_and_iname_rejected(void **state) {
  ...
  bc_runtime_cli_parsed_t parsed;
  assert_int_equal(parse_into(fixture, argv, 5, &parsed),
                   BC_RUNTIME_CLI_PARSE_OK);
  bc_seek_cli_options_t options;
...
static void test_bind_options_min_exceeds_max(void **state) {
  fixture_t *fixture = *state;
  const char *argv[] = {"bc-seek", "find", "--max-depth=2", "--min-depth=5",
                        "/tmp"};
  bc_runtime_cli_parsed_t parsed;
  assert_int_equal(parse_into(fixture, argv, 5, &parsed),
                   BC_RUNTIME_CLI_PARSE_OK);
...
int main(void) {
  const struct CMUnitTest tests[] = {
      cmocka_unit_test_setup_teardown(test_bind_global_threads_auto, setup,
                                      teardown),
      cmocka_unit_test_setup_teardown(test_bind_global_threads_mono, setup,
                                      teardown),
      cmocka_unit_test_setup_teardown(test_bind_global_threads_explicit, setup,
                                      teardown),
      cmocka_unit_test_setup_teardown(test_bind_options_defaults, setup,
                                      teardown),
      cmocka_unit_test_setup_teardown(test_bind_options_all_predicates, setup,
                                      teardown),
      cmocka_unit_test_setup_teardown(test_bind_options_iname_case_insensitive,
                                      setup, teardown),
      cmocka_unit_test_setup_teardown(test_bind_options_name_and_iname_rejected,
                                      setup, teardown),
      cmocka_unit_test_setup_teardown(test_bind_options_invalid_type, setup,
                                      teardown),
      cmocka_unit_test_setup_teardown(test_bind_options_min_exceeds_max, setup,
                                      teardown),
  };
