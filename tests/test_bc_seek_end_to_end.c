...
static bool run_capture(char *const argv[], command_result_t *out_result) {
  ...
  for (;;) {
    ...
    ssize_t bytes_read =
        read(pipe_fd[0], buffer + length, capacity - length - 1);
    if (bytes_read < 0) {
      if (errno == EINTR) {
        continue;
      }
...
static bool buffer_contains_path(const char *buffer, size_t length,
                                 const char *path) {
...
static int tree_setup(void **state) {
  ...
  snprintf(fixture->root, sizeof(fixture->root), "%s/e2e-XXXXXX",
           BC_SEEK_TEST_FIXTURES_DIRECTORY);
  ...
  if (!make_file(path, "alpha contents exceeding forty eight bytes for size "
                       "test ........")) {
    return -1;
  }
...
static void test_walk_default_ignores_hidden_and_git(void **state) {
  tree_fixture_t *fixture = *state;
  char *const argv[] = {(char *)BC_SEEK_TEST_BINARY_PATH, "find", "--type=f",
                        fixture->root, NULL};
  command_result_t result;
  assert_true(run_capture(argv, &result));
  assert_int_equal(result.exit_code, 0);
  assert_true(buffer_contains_path(result.stdout_buffer, result.stdout_length,
                                   "top.txt"));
  assert_true(buffer_contains_path(result.stdout_buffer, result.stdout_length,
                                   "alpha.c"));
  assert_true(buffer_contains_path(result.stdout_buffer, result.stdout_length,
  ...
  assert_true(buffer_contains_path(result.stdout_buffer, result.stdout_length,
                                   "gamma.c"));
  assert_false(buffer_contains_path(result.stdout_buffer, result.stdout_length,
                                    ".hidden"));
  assert_false(buffer_contains_path(result.stdout_buffer, result.stdout_length,
                                    ".git/config"));
  command_result_free(&result);
...
static void test_walk_hidden_flag(void **state) {
  tree_fixture_t *fixture = *state;
  char *const argv[] = {(char *)BC_SEEK_TEST_BINARY_PATH,
                        "find",
                        "--hidden",
                        "--type=f",
                        fixture->root,
...
static void test_walk_name_glob(void **state) {
  tree_fixture_t *fixture = *state;
  char *const argv[] = {(char *)BC_SEEK_TEST_BINARY_PATH, "find", "--name=*.c",
                        fixture->root, NULL};
  command_result_t result;
  assert_true(run_capture(argv, &result));
  assert_int_equal(result.exit_code, 0);
...
static void test_walk_null_terminated(void **state) {
  ...
  assert_true(
      count_null_separators(result.stdout_buffer, result.stdout_length) >= 3u);
  command_result_free(&result);
}
...
static void split_lines_sorted(char *buffer, size_t length, char ***out_lines,
                               size_t *out_count) {
  size_t line_count = 0;
  for (size_t index = 0; index < length; index++) {
    if (buffer[index] == '\n') {
...
static void test_walk_follow_symlinks(void **state) {
  ...
  char *const without[] = {(char *)BC_SEEK_TEST_BINARY_PATH, "find", "--type=f",
                           fixture->root, NULL};
  command_result_t result_without;
  assert_true(run_capture(without, &result_without));
  assert_int_equal(result_without.exit_code, 0);
  assert_false(buffer_contains_path(result_without.stdout_buffer,
                                    result_without.stdout_length,
                                    "sub_link/alpha.c"));
  command_result_free(&result_without);

  char *const with[] = {(char *)BC_SEEK_TEST_BINARY_PATH,
                        "find",
                        "--follow-symlinks",
                        "--type=f",
                        fixture->root,
  ...
  assert_true(buffer_contains_path(result_with.stdout_buffer,
                                   result_with.stdout_length,
                                   "sub_link/alpha.c"));
...
static void test_walk_mono_equals_parallel(void **state) {
  tree_fixture_t *fixture = *state;
  char *const mono_argv[] = {(char *)BC_SEEK_TEST_BINARY_PATH,
                             "--threads=0",
                             "find",
                             "--hidden",
                             "--no-ignore",
                             fixture->root,
                             NULL};
  char *const multi_argv[] = {(char *)BC_SEEK_TEST_BINARY_PATH,
                              "--threads=auto",
                              "find",
                              "--hidden",
                              "--no-ignore",
  ...
  split_lines_sorted(mono.stdout_buffer, mono.stdout_length, &mono_lines,
                     &mono_count);
  split_lines_sorted(multi.stdout_buffer, multi.stdout_length, &multi_lines,
                     &multi_count);
...
int main(void) {
  const struct CMUnitTest tests[] = {
      ...
      cmocka_unit_test_setup_teardown(test_walk_default_ignores_hidden_and_git,
                                      tree_setup, tree_teardown),
      cmocka_unit_test_setup_teardown(test_walk_hidden_flag, tree_setup,
                                      tree_teardown),
      cmocka_unit_test_setup_teardown(test_walk_name_glob, tree_setup,
                                      tree_teardown),
      cmocka_unit_test_setup_teardown(test_walk_max_depth, tree_setup,
                                      tree_teardown),
      cmocka_unit_test_setup_teardown(test_walk_min_depth, tree_setup,
                                      tree_teardown),
      cmocka_unit_test_setup_teardown(test_walk_size_filter, tree_setup,
                                      tree_teardown),
      cmocka_unit_test_setup_teardown(test_walk_null_terminated, tree_setup,
                                      tree_teardown),
      cmocka_unit_test_setup_teardown(test_walk_no_ignore, tree_setup,
                                      tree_teardown),
      cmocka_unit_test_setup_teardown(test_walk_mono_thread, tree_setup,
                                      tree_teardown),
      cmocka_unit_test_setup_teardown(test_walk_follow_symlinks, tree_setup,
                                      tree_teardown),
      cmocka_unit_test_setup_teardown(test_walk_mono_equals_parallel,
                                      tree_setup, tree_teardown),
  };
