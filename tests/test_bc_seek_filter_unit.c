...
static void make_candidate(bc_seek_candidate_t* candidate, const char* path, const char* basename, size_t depth, bc_seek_entry_type_t type)
{
...
static void test_evaluate_no_filters_accepts(void** state)
{
    ...
    assert_int_equal(bc_seek_filter_evaluate(&predicate, &candidate), BC_SEEK_FILTER_DECISION_ACCEPT);
}
...
static void test_evaluate_name_filter(void** state)
{
    ...
    assert_int_equal(bc_seek_filter_evaluate(&predicate, &match), BC_SEEK_FILTER_DECISION_ACCEPT);

    bc_seek_candidate_t no_match;
    make_candidate(&no_match, "/tmp/foo.h", "foo.h", 1u, BC_SEEK_ENTRY_TYPE_FILE);
    assert_int_equal(bc_seek_filter_evaluate(&predicate, &no_match), BC_SEEK_FILTER_DECISION_REJECT);
}
...
static void test_evaluate_type_filter(void** state)
{
    ...
    make_candidate(&dir_candidate, "/tmp/d", "d", 1u, BC_SEEK_ENTRY_TYPE_DIRECTORY);
    assert_int_equal(bc_seek_filter_evaluate(&predicate, &dir_candidate), BC_SEEK_FILTER_DECISION_ACCEPT);

    bc_seek_candidate_t file_candidate;
    make_candidate(&file_candidate, "/tmp/f", "f", 1u, BC_SEEK_ENTRY_TYPE_FILE);
    assert_int_equal(bc_seek_filter_evaluate(&predicate, &file_candidate), BC_SEEK_FILTER_DECISION_REJECT);
}
...
static void test_evaluate_depth_filter(void** state)
{
    ...
    assert_int_equal(bc_seek_filter_evaluate(&predicate, &too_shallow), BC_SEEK_FILTER_DECISION_REJECT);

    bc_seek_candidate_t too_deep;
    make_candidate(&too_deep, "/a/b/c/d", "d", 4u, BC_SEEK_ENTRY_TYPE_FILE);
    assert_int_equal(bc_seek_filter_evaluate(&predicate, &too_deep), BC_SEEK_FILTER_DECISION_REJECT);

    bc_seek_candidate_t in_range;
    make_candidate(&in_range, "/a/b/c", "c", 3u, BC_SEEK_ENTRY_TYPE_FILE);
    assert_int_equal(bc_seek_filter_evaluate(&predicate, &in_range), BC_SEEK_FILTER_DECISION_ACCEPT);
}
...
static void test_evaluate_size_requires_stat(void** state)
{
    ...
    assert_int_equal(bc_seek_filter_evaluate(&predicate, &stat_populated), BC_SEEK_FILTER_DECISION_ACCEPT);
    ...
    assert_int_equal(bc_seek_filter_evaluate(&predicate, &stat_populated_small), BC_SEEK_FILTER_DECISION_REJECT);
}
...
static void test_evaluate_path_filter(void** state)
{
    ...
    bc_seek_candidate_t match;
    make_candidate(&match, "/a/tests/foo.c", "foo.c", 2u, BC_SEEK_ENTRY_TYPE_FILE);
    assert_int_equal(bc_seek_filter_evaluate(&predicate, &match), BC_SEEK_FILTER_DECISION_ACCEPT);

    bc_seek_candidate_t no_match;
    make_candidate(&no_match, "/a/src/foo.c", "foo.c", 2u, BC_SEEK_ENTRY_TYPE_FILE);
    assert_int_equal(bc_seek_filter_evaluate(&predicate, &no_match), BC_SEEK_FILTER_DECISION_REJECT);
}
...
int main(void)
{
    const struct CMUnitTest tests[] = {
        cmocka_unit_test(test_ignored_directory_names),     cmocka_unit_test(test_glob_matches),
        cmocka_unit_test(test_evaluate_no_filters_accepts), cmocka_unit_test(test_evaluate_name_filter),
        cmocka_unit_test(test_evaluate_type_filter),        cmocka_unit_test(test_evaluate_depth_filter),
        cmocka_unit_test(test_evaluate_size_requires_stat), cmocka_unit_test(test_evaluate_path_filter),
    };
